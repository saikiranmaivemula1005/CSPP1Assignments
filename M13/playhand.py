...
def updateHand(hand, word):
    ...
    hand_up = dict(hand)
    for i in word:
        if i in hand_up.keys():
            hand_up[i] = hand_up[i] - 1
    return hand_up
#
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while word != '.':
        if isValidWord(word, hand, wordList) == True:
            ...
            updated_hand = updateHand(hand, word)
            print(displayHand(updated_hand))
            if calculateHandlen(updated_hand) == 0:
                print('game ended')
                break
            print('enter a word')
            word = input()
